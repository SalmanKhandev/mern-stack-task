...
export default function Homepage(props) {
  ...
  const searchProduct = async (name) => {
    try {
      console.log(name);
      const result = await fetch(`/api/product/filterproduct/${name}`);
      const response = await result.json();

      // console.log(result);
      setFilteredState(response);
  ...
  useEffect(() => {
    if (search.length > 0 && initialState) {
      console.log(search);
      searchProduct(search);

      // searchProduct("Samsung Galaxy S1");
